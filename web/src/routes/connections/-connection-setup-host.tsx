...
export function BrokeringPage({ setupId }: { setupId: string }) {
    ...
    const connect = (event: MessageEvent) => {
      ...
      port.onmessage = async ({ data }) => {
        ...
        try {
          ...
          else if (data.method === "setConnectionName") {
            if (
              typeof data.connectionName !== "string" ||
              typeof data.actionId !== "string" ||
              data.connectionName.length > 200
            )
              throw new Error("Invalid account name");
            response = await client.setConnectionName({
              setupId,
          ...
          if (data.method === "read") setFrameReady(true);
        } catch (error) {
          setFrameReady(true);
          port?.postMessage({
            id: data.id,
  ...
  useEffect(() => {
    if (!state || !["complete", "cancelled"].includes(state.action.case ?? "")) return;
    const notification = {
      type:
        state.action.case === "complete"
          ? "tilde.connection.complete"
          : "tilde.connection.cancelled",
      connectionId: state.connectionId,
    };
    // Contains no credentials. Receivers verify this window, origin and connection ID.
    window.opener?.postMessage(notification, "*");
    if (window.parent !== window) window.parent.postMessage(notification, "*");
  }, [state]);
  ...
  return (
    <LoadingReveal loading={!frameReady && !frameError && !error} label="Loading connection setup">
      {(frameError || error) && (
        <p role="alert">{error || "The provider UI did not respond. You can reload this page."}</p>
      )}
      {state && (
        <iframe
          ref={frame}
