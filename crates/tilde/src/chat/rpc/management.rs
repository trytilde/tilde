...
use crate::services::tilde::management::v1::ChatService;
...
impl ChatService for Rpc {
    ...
    ) -> ServiceResult<impl connectrpc::Encodable<management::ListThreadsResponse> + Send + use<'a>>
    {
        ...
        let agent = r.agent_id.map(id).transpose()?;
        ...
        Response::ok(management::ListThreadsResponse {
            threads: rows
                ...
                .map(|r| types::Thread {
                    id: r.id.to_string(),
                    title: r.title,
                    primary_agent_id: r.primary_agent_id.to_string(),
                    ..Default::default()
                })
                .collect(),
            next_page_token,
    ...
    {
        {
            let page = self
                .0
                .activity_page(id(r.thread_id)?, r.after_sequence, r.limit)
                .await?;
    ...
    > {
        ...
        Response::stream_ok(async_stream::try_stream! {
            loop {
                ...
                let page=chat.activity_page(thread,cursor,100).await?;
                for event in page.events {cursor=event.sequence;yield management::WatchThreadResponse{activity:event.into(),..Default::default()};}
