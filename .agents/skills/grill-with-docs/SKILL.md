...
When a resolved decision changes or materially clarifies an existing ADR,
update its current prose and append the timestamped `## Updates` entry required
by `ADR-FORMAT.md`. Preserve every older update entry.
