...
test -x .tools/corrosion
tar -czf "dist/release/$archive" -C "$binary_dir" tilde tilde-sidecar -C "$release_root/.tools" corrosion corrosion.LICENSE corrosion.revision
(
  cd dist/release
