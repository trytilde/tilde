WITH eligible AS (
 ...
 WHERE i.id=$1 AND i.status='pending' AND a.deployment_mode='gateway' AND NOT a.paused AND a.deleted_at IS NULL AND chat_channel_run_allowed(i.agent_id,i.thread_id,r.source_identity_id,r.channel_origin) FOR SHARE OF a
)
